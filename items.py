...
def itemLookup(id, type, count, isReward):
	if(type == "item"):
		...
		jsonEquip = requests.get("https://api.atlasacademy.io/basic/JP/equip/" + str(id) + "?lang=en").json()
		if jsonEquip["type"] == "servantEquip":
			if count == 1:
				return " {{Craft Essence Icon|" + jsonEquip["name"] + "|" + str(jsonEquip["collectionNo"]) + "|45}}"
			else:
				return " {{Craft Essence Icon|" + jsonEquip["name"] + "|" + str(jsonEquip["collectionNo"]) + "|45|inum=" + '{:,}'.format(count) + "}}"
		elif jsonEquip["type"] == "combineMaterial":
			if isReward:
				servEquip = requests.get("https://api.atlasacademy.io/basic/JP/servant/" + str(id)).json()
				if count == 1:
					return " {{Exp Icon|rarity=" + str(servEquip["rarity"]) + "|class=" + servEquip["className"].lower() + "}}"
				else:
					return " {{Exp Icon|rarity=" + str(servEquip["rarity"]) + "|class=" + servEquip["className"].lower() + "|" + '{:,}'.format(count) + "}}"
			else:
				return ""
		elif jsonEquip["type"] == "statusUp":
			if isReward:
				servEquip = requests.get("https://api.atlasacademy.io/basic/JP/servant/" + str(id)).json()
				niceEquip = requests.get("https://api.atlasacademy.io/nice/JP/equip/" + str(id)).json()
				if niceEquip["atkGrowth"][0] > 0 and niceEquip["hpGrowth"][0] == 0:
					fouType = "atk"
				elif niceEquip["hpGrowth"][0] > 0 and niceEquip["atkGrowth"][0] == 0:
					fouType = "hp"
				else:
					fouType = "<INDETERMINATE>"
				if count == 1:
					return " {{Status Up Icon|type=" + fouType + "|rarity=" + str(servEquip["rarity"]) + "|class=" + servEquip["className"].lower() + "}}"
				else:
					return " {{Status Up Icon|type=" + fouType + "|rarity=" + str(servEquip["rarity"]) + "|class=" + servEquip["className"].lower() + "|" + '{:,}'.format(count) + "}}"
			else:
				return ""
		else:
			return " Item "+ str(id) + " (" + jsonEquip["name"] + ") ×" + '{:,}'.format(count)
